"""
Ground Truth Validation: Synthetic Experiments
=========================================================

This script validates the H-FD (Hilbert-valued Functional Decomposition)
framework on a synthetic ICU-inspired model with known analytical ground truth.

Data-generating process (DGP)
------------------------------
The true model maps three scalar features to a time-valued output trajectory:

    F(x)(t) = x1*phi1(t) + x2*phi2(t) + x3*phi3(t)
            + alpha*(x1 - mu)*(x2 - mu)*phi12(t)

where the basis functions are:
    phi1(t)  = exp(-0.2*t)                    — decaying exponential
    phi2(t)  = exp(-(t-10)^2 / 2)             — Gaussian bump at t=10
    phi3(t)  = exp(-(t-18)^2 / 2)             — Gaussian bump at t=18
    phi12(t) = exp(-(t-5)^2  / 2)             — pairwise interaction at t=5

Features are i.i.d. Uniform[0,1] with center mu=0.5 and interaction
strength alpha=1.0. Observations are corrupted with correlated
(Ornstein–Uhlenbeck) Gaussian noise.

Experiment design
-----------------
Four predictive models are trained on noisy observations:
    M1: Ridge regression          (linear, intentionally misspecified)
    M2: Random Forest
    M3: NGBoost                   (independent Normal per time point)
    M4: MLP                       (direct trajectory prediction)

An Oracle baseline applies Möbius inversion to the TRUE model with n
background samples (no learning error — only estimation error).

For each model, pure-effect functions {f_S} are estimated via Möbius
inversion on the functional game values. These are compared against the
analytically known ground truth.

Error metrics
-------------
Two complementary metrics assess recovery quality:
    - Normalized L2 error:       curve-level recovery of f_S(t)
    - Relative aggregated error: scalar integral Phi_S = integral f_S(t) dt

Outputs
-------
Figures and CSV tables are saved to:
    plots/synthetic_experiments/gt_validation/

Usage
-----
Full run (30 seeds, all n values):
    python gt_validation.py

Quick run (1 seed, reduced n values — for debugging):
    python gt_validation.py --quick

Regenerate figures from cached results only:
    python gt_validation.py --plots_only

All CLI options:
    --n_runs      Number of random seeds / repetitions  (default: 30)
    --seed        Master random seed                    (default: 0)
    --latent_dim  MLP latent dimension                  (default: 8)
    --n_est       NGBoost base estimator count          (default: 200)
    --n_jobs      Parallel workers for experiment loop  (default: 32)
    --rf_jobs     Parallel workers inside RandomForest  (default: 4)
    --device      Torch device: 'cpu' or 'cuda'         (default: cpu)
    --gpu_id      CUDA device index                     (default: 0)
    --quick       Use reduced n_values and n_runs=1
    --plots_only  Skip experiments; load cache and plot
    --cache_dir   Directory for .npz result cache
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description='H-FD: Synthetic ground-truth validation.')
    ...
    p.add_argument('--latent_dim', type=int, default=8,
                   help='Latent dimension of the MLP encoder.')
    p.add_argument('--n_est',      type=int, default=200,
                   help='Base number of estimators for NGBoost.')
    ...
    p.add_argument('--gpu_id',     type=int, default=0,
                   help='CUDA device index (ignored if --device cpu).')
    p.add_argument('--quick',      action='store_true',
                   help='Use reduced n_values and n_runs=1 for fast debugging.')
    p.add_argument('--cache_dir',  type=str,
                   default=os.path.join(
                       'plots', 'synthetic_experiments', 'gt_validation', 'cache'),
                   help='Directory for .npz result cache.')
    p.add_argument('--plots_only', action='store_true',
                   help='Skip experiments; load cache and regenerate plots.')
    return p.parse_args()
...
PLOT_DIR = os.path.join('plots', 'synthetic_experiments', 'gt_validation')
...
# DGP parameters
MU    = 0.5   # centering constant for feature values
ALPHA = 1.0   # interaction strength

# Training sizes to sweep (full and quick variants)
N_VALUES_FULL  = [50, 100, 250, 500, 1000, 2000, 5000, 10000]
N_VALUES_QUICK = [50, 250, 1000, 2000]

# The n value used as the "representative" run for effect plots
REP_N = 1000

# OU noise correlation length (controls temporal smoothness of noise)
NOISE_ELL = 2.0

# Fixed evaluation point x* for effect comparison figures
X_STAR = np.array([0.8, 0.9, 0.7])

# Subsets S ⊆ {1,2,3} whose pure effects f_S are reported
# (1,2) is the only nonzero pairwise interaction in the DGP
REPORT_SUBSETS = [(1,), (2,), (3,), (1, 2)]

# Display labels for subsets (LaTeX and plain)
SUBSET_LABELS = {
    (1,):   r'$f_{\{X_1\}}$',
    (2,):   r'$f_{\{X_2\}}$',
    (3,):   r'$f_{\{X_3\}}$',
...
# Display labels, colors, linestyles, and markers per model
MODEL_LABELS = {
...
# Basis functions defining the true DGP
...
def model_true(X, t):
    """
    Evaluate the true functional model F(x)(t) for a batch of inputs X.
...
def analytical_pure_effects(t):
    """
    Compute the analytical pure Möbius effects f_S(t) at X_STAR.

    These are derived by centering features around mu=0.5 and exploiting
    the additive+pairwise structure of the DGP. All higher-order and
    cross effects not listed here are exactly zero.

    Returns
    -------
    dict mapping subset tuples -> ndarray of shape (T,)
    """
    x1, x2, x3 = X_STAR
    return {
        (1,):      (x1 - MU) * phi1(t),
...
def analytical_aggregated(effects_dict, t):
    """
    Compute scalar time-aggregated effects Phi_S = integral f_S(t) dt
    via the trapezoidal rule.

    Returns
    -------
    dict mapping subset tuples -> float
    """
...
def compute_signal_variance(t, n_mc=20000, rng=None):
    """
    Estimate the average marginal variance of F(x)(t) over t via Monte Carlo.
    Used to set the noise level relative to signal strength (SNR).
    """
    if rng is None:
...
def compute_variance_decomposition(alpha, t, n_mc=50000, seed=0):
    """
    Estimate the functional variance decomposition of the DGP.

    Decomposes total trajectory variance into contributions from each
    main effect and the pairwise interaction term. Returns fractional
    contributions and total variance.

    Returns
    -------
    dict with keys 'f1', 'f2', 'f3', 'f12', 'vtot'
    """
...
# ===========================================================================
# 2.  Noise generation (Ornstein–Uhlenbeck covariance)
# ===========================================================================

def make_ou_cov(t, sigma2, ell):
    """
    Construct the OU covariance matrix K(s,t) = sigma2 * exp(-|s-t| / ell).
...
def sample_noise(t, sigma2, ell, n, rng):
    """
    Draw n independent noise trajectories from the OU process via
    Cholesky factorization.

    Returns
    -------
    eps : ndarray of shape (n, T)
    """
...
def generate_training_data(n, t, sigma2, ell, rng):
    """
    Sample n training observations (X, Y) where Y = F(X)(t) + noise.

    Returns
    -------
    X : ndarray of shape (n, 3)
    Y : ndarray of shape (n, T)  — noisy trajectory observations
    """
...
def make_ridge():
    """
    Wrap Ridge regression in MultiOutputRegressor for trajectory prediction.
    Intentionally misspecified: cannot capture the pairwise interaction term.
    """
...
def make_ngboost_direct(n_est, random_state):
    """
    NGBoost with Normal distribution, wrapped in MultiOutputRegressor.
    Treats each time point independently — no temporal coupling.
    """
...
def get_ngboost_n_est(n, base):
    """
    Reduce NGBoost estimator count for large n to keep runtime feasible.
    Scales down in two steps beyond n=500 and n=2000.
    """
...
# ---------------------------------------------------------------------------
# MLP: encoder-decoder architecture for direct trajectory regression
# ---------------------------------------------------------------------------

class _MLPNet(nn.Module):
    """
    Encoder-decoder MLP for trajectory prediction.

    Architecture:
        encoder: input (p=3) -> hidden layers -> latent_dim
        decoder: latent_dim  -> hidden layers -> T time points

    Activation: Tanh throughout (smooth, bounded — suitable for
    regression on bounded functional outputs).
    """
...
class _MLPModel:
    """
    Training wrapper for _MLPNet.

    Includes:
        - Adam optimizer with ReduceLROnPlateau scheduler
        - 15% validation split with early stopping
    ...
    def fit(self, X, Y):
        ...
        for _ in range(self.n_epochs):
            ...
            self.net.train()
            scheduler.step(vl)
            # Early stopping: save best weights, stop after `patience` epochs
            if vl < best_val - 1e-7:
                best_val   = vl
...
def _mlp_capacity(n):
    """
    Select MLP architecture and training hyperparameters based on dataset size.

    Larger n warrants deeper/wider networks and more training epochs.
    Returns: (hidden_enc, hidden_dec, n_epochs, batch_size, patience)
    """
    if n < 200:
...
def make_mlp_direct(n, T, latent_dim, rng, device='cpu'):
    """Instantiate an _MLPModel with capacity scaled to n."""
...
# ===========================================================================
# 4.  Möbius inversion for functional game decomposition
# ===========================================================================

def subset_to_mask(S, p=3):
    """
    Convert a feature subset tuple S ⊆ {1,...,p} to a boolean mask
    of length p (0-indexed).
    """
...
def impute_x(x_star, X_bg, S_mask):
    """
    Construct an imputed version of the background set X_bg where
    features in S (indicated by S_mask) are fixed to x_star.

    This is the standard marginal imputation used in the functional
    cooperative game v(S) = E[F(x_S, X_{-S}) | x_S = x_star_S].

    Parameters
    ----------
    x_star : ndarray of shape (p,)
    X_bg   : ndarray of shape (n_bg, p)  — background samples
    S_mask : bool ndarray of shape (p,)

    Returns
    -------
    x_imp : ndarray of shape (n_bg, p)
    """
    x_imp = X_bg.copy()
...
def compute_game_values(predict_fn, x_star, X_bg, t):
    """
    Compute the functional game values v(S)(t) for all S ⊆ {1,2,3}.

    For each coalition S:
        v(S)(t) = mean over X_bg of predict_fn(x_S = x_star_S, X_{-S})

    The empty coalition v({})(t) is the overall background mean prediction.

    Parameters
    ----------
    predict_fn : callable, maps (n, p) -> (n, T)
    x_star     : ndarray of shape (p,)
    X_bg       : ndarray of shape (n_bg, p)
    t          : ndarray of shape (T,)
    ...
    for S in all_S:
        if len(S) == 0:
            # Baseline: unconditional background mean
...
def mobius_transform(v_dict, p=3):
    """
    Apply the Möbius inversion formula to recover pure effects from
    coalition values.

    The Möbius transform inverts the inclusion-exclusion relationship:
        v(S)(t) = sum_{L ⊆ S} m(L)(t)

    giving:
        m(S)(t) = sum_{L ⊆ S} (-1)^{|S|-|L|} v(L)(t)

    Each m(S)(t) is the pure effect of subset S — the part of the model
    output attributable exclusively to interactions within S.

    Parameters
    ----------
    v_dict : dict mapping subset tuples -> ndarray of shape (T,)
...
def estimate_mobius(predict_fn, x_star, X_bg, t):
    """
    Full pipeline: compute game values then apply Möbius inversion.

    Returns
    -------
    m : dict mapping subset tuples -> ndarray of shape (T,)
        Pure functional effects at x_star.
    """
...
def l2_error_normalized(est, truth, t):
    """
    Normalized L2 error between estimated and true functional effect:

        || est - truth ||_2 / || truth ||_2

    where norms are computed via numerical integration (trapezoidal rule).
    A value of 0 indicates perfect curve recovery; 1 means the error is
...
def aggregated_error(est_scalar, truth_scalar):
    """
    Relative error on the time-aggregated (scalar) effect:

        |Phi_S^hat - Phi_S| / |Phi_S|

    where Phi_S = integral f_S(t) dt.
...
# ===========================================================================
# 6.  Single experimental run (one seed, one n)
# ===========================================================================

def run_one(n, seed, t, sigma2, latent_dim, base_n_est,
            rf_jobs, device):
    """
    Execute one complete experimental run:
        1. Generate training data (n samples, correlated noise)
        2. Fit all four models (Ridge, RF, NGBoost, MLP)
        3. Estimate Möbius effects for all models + Oracle
        4. Compute L2 and aggregated errors against analytical ground truth
    ...
    latent_dim : int   — MLP latent dimension
    base_n_est : int   — base NGBoost estimator count
    rf_jobs    : int   — RF parallelism
    device     : str   — 'cpu' or 'cuda'

    Returns
    -------
    out : dict with keys (tag, S) -> {'l2_err', 'agg_err', 'effect'}
          and '_truth_effects', '_truth_agg', '_mobius'
    """
    os.environ['CUDA_VISIBLE_DEVICES'] = ''
    warnings.filterwarnings('ignore')
    ...
    # Oracle uses the true model — isolates estimation error from
    # the background sample size, with no model learning error
    ...
    # Estimate Möbius effects for each model
    mobius = {}
    ...
    # Compute error metrics for each (model, subset) pair
...
# ===========================================================================
# 7.  Parallel experiment loop (sweep over n and seeds)
# ===========================================================================

def run_experiments(n_runs, base_seed, t, sigma2, latent_dim, n_est,
                    n_values, n_jobs, rf_jobs, device, cache_dir):
    """
    Run the full experiment grid: all (n, seed) combinations in parallel
    using joblib. Aggregates per-run results into arrays indexed by
    (n, model, subset).

    Parameters
    ----------
    n_runs    : int   — number of independent seeds per n value
    base_seed : int   — master seed for generating per-run seeds
    n_values  : list  — training sizes to sweep
    n_jobs    : int   — total parallel workers

    Returns
    -------
    l2_errors  : nested dict [n][tag][S] -> ndarray of shape (n_runs,)
    agg_errors : nested dict [n][tag][S] -> ndarray of shape (n_runs,)
    representative : dict — results from the REP_N run (for effect plots)
    """
    ...
    # Aggregate results into per-(n, model, subset) error arrays
...
# ===========================================================================
# 8.  Cache helpers (save/load .npz result files)
# ===========================================================================

def _save_cache(l2_errors, agg_errors, representative,
                n_values, cache_dir):
    """
    Save error arrays and representative run effects to .npz files.

    One file per n value (results_n{n}.npz) plus a combined
    representative.npz for effect comparison figures.
    """
    os.makedirs(cache_dir, exist_ok=True)
    for n in n_values:
        d = {}
...
def _load_cache(n_values, cache_dir):
    """
    Load previously saved error arrays and representative run from cache.
    Used when --plots_only is set to regenerate figures without re-running.
    """
...
def _style_ax(ax, ylabel=None, xlabel='Time (h)'):
    """
    Apply standard axis styling for time-domain effect plots.
    Adds light shading for the two ICU activity windows (08–12h, 16–20h).
    """
    ax.set_xlim(0, 24)
    ...
    # Light background shading for illustrative time windows
...
def _n_recovery_panel(ax, error_dict, n_values, tag_list,
    ...
    """
    Plot error vs. n curves for all models on a single axes panel.

    Draws mean ± 1 std bands on a log-scale x-axis.
    A horizontal dashed line at error=1.0 marks the "trivial" threshold
    (error as large as the signal).
    """
    for tag in tag_list:
    ...
    # Reference line: error = 1 (no better than zero estimate)
...
# ===========================================================================
# 10.  Fig 1 — Combined N-recovery: L2 (top row) + Aggregated (bottom row)
# ===========================================================================
...
def plot_n_recovery_combined(l2_errors, agg_errors, n_values, n_runs,
    ...
    """
    Generate Fig 1: two-row grid showing how functional effect recovery
    improves with training set size n.

    Row 0: Normalized L2 error (curve-level recovery)
    Row 1: Relative aggregated error (scalar integral recovery)
    Columns: one per reported subset S

    A single shared legend is placed between the two rows.
    """
    ...
    fig.suptitle(
        f'N-Recovery: Normalized L2 error (top) and '
    ...
    row_labels  = ['Normalized L2 error', 'Relative aggregated error']
    ...
    savefig(fig, 'fig1_n_recovery_combined.pdf', plot_dir)


# ===========================================================================
# 11.  Effect comparison helpers (for Fig 2)
# ===========================================================================
...
def _effect_comparison_panels(axes, representative, t,
                               ...
                               subset_colors):
    """
    Plot time-resolved pure effects f_S(t) for all models vs. analytical
    ground truth, one axes panel per reported subset S.
    Oracle is excluded from the comparison panels (shown in aggregated bars).
    """
    ...
    for col, S in enumerate(report_subsets):
        ...
        # Estimated effects from each trained model
        for tag in tags:
        ...
        # Analytical ground truth as dashed reference
        ax.plot(t, truth,
                color='#888888', lw=1.4, ls='--',
                dashes=(4, 3), label='Analytical', zorder=10)

        _style_ax(ax,
                  ylabel='Time-resolved pure effect' if col == 0 else None)
        ax.set_title(subset_labels[S], fontsize=FS_TITLE,
                     fontweight='bold', color=subset_colors[S])
        if col == 0:
...
def _aggregated_effects_panel(ax, representative, t):
    """
    Bar chart comparing time-aggregated effects Phi_S = integral f_S(t) dt
    across all models and the analytical ground truth.

    One group of bars per subset S, with one bar per model/oracle/analytical.
    """
    truth_agg = {
...
# ===========================================================================
# 12.  Fig 2 — Effects combined: time-resolved (top) + aggregated bars (bottom)
# ===========================================================================
...
def plot_effects_combined(representative, t, n_values,
                          ...
                          subset_colors, rep_n):
    """
    Generate Fig 2: two-row layout for a representative run at n=REP_N.

    Row 0: Time-resolved pure effects f_S(t) for each subset S
    Row 1: Time-aggregated scalar effects Phi_S (bar chart, spanning all cols)

    The representative run is the last seed processed at n=REP_N.
    """
    ...
    # Row 0: time-resolved effect curves per subset
    ...
    savefig(fig, 'fig2_effects_combined.pdf', plot_dir)


# ===========================================================================
# 13.  CSV summary tables
# ===========================================================================

def build_summary_table(l2_errors, agg_errors, n_values):
    """
    Build a compact summary table showing mean ± std for both metrics
    at the largest n value, one row per model.
    """
    n_large = n_values[-1]
    rows    = []
    for tag in ALL_MODELS:
...
def build_full_table(error_dict, n_values, metric_name):
    """
    Build a full table of mean ± std for a single metric,
    indexed by (model, subset), with one column per n value.
    """
    rows = []
    for tag in ALL_MODELS:
        for S in REPORT_SUBSETS:
...
def build_convergence_table(l2_errors, agg_errors, n_values):
    """
    Build a convergence table showing both metrics at four selected
    n values (small, medium, large, largest), for each (model, subset).
    Useful for assessing the rate of convergence with sample size.
    """
...
def write_csv_tables(l2_errors, agg_errors, n_values, n_runs):
    """
    Write all four summary CSV tables to plots/.../csv/.
    No figure rendering — pure data export.
    """
...
def main():
    ...
    # Log DGP variance fractions for transparency
    ...
    log.info('Generating combined figures ...')
    ...
    write_csv_tables(l2_errors, agg_errors, n_values, n_runs)
