...
Corrections vs previous version:
  1. Network plots: edge weights reflect effect type (pure/partial/full)
  2. Profile comparison: per-feature, per-profile kernel assignment
       High-VIX Announcement: causal(ell=8) for ann_indicator, OU(ell=8) for rest
       Quiet Low-VIX:         identity for ann_indicator (zero, irrelevant),
                               identity for day_of_week/month (discrete),
                               OU(ell=8) for vix_prev/trailing_rv/overnight_ret
       Monday Gap:            causal(ell=8) for overnight_ret (directional gap),
                               identity for ann_indicator,
                               OU(ell=8) for vix_prev/trailing_rv,
                               identity for day_of_week/month
  3. Font sizes increased throughout for readability
"""
...
#
#  The kernel choice should reflect the temporal structure of each feature's
#  effect on a given profile type:
#
#  High-VIX Announcement:
#    ann_indicator  -> causal (ell=8): 14:00 event, no backward leakage
#    all others     -> OU (ell=8): sustained intraday mean-reversion
#
#  Quiet Low-VIX (non-announcement, low volatility):
#    ann_indicator  -> identity: feature is zero, kernel irrelevant;
#                                identity avoids spurious smoothing of noise
#    day_of_week    -> identity: discrete ordinal, no temporal neighbourhood
#    month          -> identity: discrete ordinal, no temporal neighbourhood
#    vix_prev       -> OU (ell=8): slow persistent decay from open
#    overnight_ret  -> OU (ell=8): gap digestion over first ~40 min
#    trailing_rv    -> OU (ell=8): regime-level persistence throughout day
#
#  Monday Gap (positive overnight gap, moderate VIX, no announcement):
#    ann_indicator  -> identity: feature is zero
#    overnight_ret  -> causal (ell=8): directional gap signal; effect
#                                      should not anticipate the gap
#    day_of_week    -> identity: discrete ordinal
#    month          -> identity: discrete ordinal
#    vix_prev       -> OU (ell=8): persistent volatility regime
#    trailing_rv    -> OU (ell=8): volatility clustering
# ===========================================================================

# Kernel type labels for legend annotation
KERNEL_TYPE_LABEL = {
    'identity': 'Identity',
    'ou'      : 'OU ($\\ell=8$)',
    'causal'  : 'Causal ($\\ell=8$)',
...
def get_profile_kernels(profile_label, pnames):
    """
    Returns a dict: feature_index -> (K_matrix, kernel_type_str)
    kernel_type_str is one of 'identity', 'ou', 'causal'
    """
    n = len(pnames)
    ...
    for r, gtype in enumerate(game_types):
        ...
        for c, kname in enumerate(kernel_names):
            ...
            for fi, col, lbl in zip(focus, colors, labels):
                ax.plot(t_grid, apply_kernel(shapley[fi], K) * sc,
                        color=col, lw=2.2,
            ...
            if c == 0:
                ...
                ax.text(-0.38, 0.5, GAME_LABELS[gtype],
                        transform=ax.transAxes, fontsize=FS_AXIS - 1,
                        va='center', ha='right', rotation=90, color='#333')
            ax.set_xlabel('Time', fontsize=FS_AXIS)
            ax.tick_params(axis='y', labelsize=FS_TICK)

    handles = [
        Line2D([0], [0], color=FEAT_COLORS['vix_prev'],      lw=2.2, label='vix_prev'),
        Line2D([0], [0], color=FEAT_COLORS['ann_indicator'],  lw=2.2, label='ann_indicator'),
        Line2D([0], [0], color='#555', lw=0.9, ls='--', alpha=0.6, label='14:00 ET'),
    ]
...
# ===========================================================================
# 9.  Figure 2 -- Main effects, pure/partial/full, identity kernel, all games
# ===========================================================================
...
# ===========================================================================
# 10. Figure 2b -- Main effects, mixed kernel, all games
# ===========================================================================

def _feature_kernel_hv(fi, pnames):
    """High-VIX profile: causal for ann_indicator, OU for rest."""
...
def plot_sensitivity_gap(moebius_sens, pnames, top_k=4):
    ...
    fig, axes = plt.subplots(1, top_k, figsize=(5.0 * top_k, 4.8), sharey=False)
    fig.suptitle(
    ...
    for idx, (ax, fi) in enumerate(zip(axes, top)):
        ...
        ax.legend(fontsize=FS_LEGEND, loc=_leg_locs[idx], framealpha=0.85)
...
# ===========================================================================
# 12. Figure 4 -- Pairwise interactions, prediction + risk, OU kernel
# ===========================================================================
...
# ===========================================================================
# 13. Figure 5 -- Profile comparison with per-profile, per-feature kernels
# ===========================================================================

def plot_profiles_comparison(profile_results, pnames):
    """
    Per-profile, per-feature kernel assignment (see get_profile_kernels).
    Each panel annotates the kernel used for the top displayed features.
    """
    n_prof    = len(profile_results)
    sc        = _scale('prediction')
    n_players = len(pnames)

    # Globally consistent feature ranking across profiles (top 4)
    ...
    profile_titles = {
        ...
        'Monday Gap':
            'Monday Gap\n(overnight gap +0.87%, VIX$\\approx$22)',  # <-- remove backslash
    }

    # Kernel legend entries per profile: collect unique (feature, ktype) pairs
    # for the top-4 features shown
    def _kernel_legend(label, top_features):
    ...
    fig, axes = plt.subplots(1, n_prof, figsize=(6.0 * n_prof, 5.0),
                             sharey=False)
    fig.suptitle(
        'Shapley curves — per-feature kernels — prediction game\n'
    ...
    for p_idx, (ax, (label, (moebius, shapley))) in enumerate(
        ...
        # Feature legend in middle panel
        if p_idx == 1:
            ax.legend(fontsize=FS_LEGEND, loc='lower right')

    # ── Linestyle legend: inside bottom center of rightmost panel ────────
    ls_handles = [
...
# ===========================================================================
# 14. Figure 0 -- Main body summary (3-row: identity / mixed-pred / mixed-risk)
# ===========================================================================
...
def plot_main_body_summary_v2(moebius_dict, shapley_dict, pnames):
    ...
    fig, axes = plt.subplots(3, 4, figsize=(19, 12.0),
                             gridspec_kw={'width_ratios': [3, 3, 3, 1.8]})
    fig.suptitle(
...
# ===========================================================================
# 15. Network helpers (CORRECTED: edge weights reflect effect type)
# ===========================================================================
...
def _network_importances_spy(moebius, shapley, n_players, K, effect_type):
    """
    Compute node importances and signed edge weights reflecting effect_type.

    Node importance: integral of |kernel-smoothed effect| for the given type.
    Edge weight (CORRECTED):
      pure:    m_{ij}(t)  -- direct pairwise Mobius coefficient
      partial: Shapley interaction -- sum_{S containing i,j} m_S / |S|
      full:    sum_{S containing i,j} m_S  -- all interactions involving both i and j
    """
    pure_eff = _pure_effects(moebius, n_players)
    full_eff = _full_effects(moebius, n_players)
    ...
    # ── Edge weights: depend on effect_type ──────────────────────────────
    edge_imp = {}
    for i in range(n_players):
        for j in range(i + 1, n_players):
            if effect_type == 'pure':
                # Direct pairwise Mobius coefficient only
                raw = moebius.get((i, j), np.zeros(T_BARS))

            elif effect_type == 'partial':
                # Shapley interaction: weighted sum over all supersets of {i,j}
                raw = np.zeros(T_BARS)
                for S, m in moebius.items():
                ...
                # All interaction mass touching both i and j
                raw = np.zeros(T_BARS)
                for S, m in moebius.items():
                    if i in S and j in S:
                        raw = raw + m
...
def plot_network_all_games_ppf(moebius_dict, shapley_dict, pnames):
    """All 3 games x pure/partial/full, OU kernel (3x3 grid). Corrected edges."""
    K_ou      = kernel_ou(t_grid, 8.0)
    n_players = len(pnames)
...
def plot_network_prediction_risk(moebius_dict, shapley_dict, pnames):
    """Prediction + risk networks, partial (Shapley), OU kernel. Corrected edges."""
    K_ou      = kernel_ou(t_grid, 8.0)
    n_players = len(pnames)
...
if __name__ == '__main__':
    print('\n' + '=' * 60)
    print('  SPY Intraday Vol  —  RF direct  (corrected)')
    print('=' * 60)
    ...
    savefig(plot_operator_sweep(moebius_hv, shapley_hv, kernels, pnames),
    ...
    savefig(plot_sensitivity_gap(moebius_hv['sensitivity'], pnames, top_k=4),
    ...
    savefig(plot_network_prediction_risk(moebius_hv, shapley_hv, pnames),
